...
function convertTime(time) {
  ...
  if (hour >= 12) {
    return <p>{`${hour - 19}:${time.substring(3, 5)}PM(PDT)`}</p>;
  } else {
    return <p>{`${Number(hour) + 5}:${time.substring(3, 5)}PM(PDT)`}</p>;
  }
}
...
function getGameState(num, data) {
  if (num == 2) {
    return (
      <div>
        <p className={styles.scoreAndState}>
    ...
    return (
      <div>
        ...
        <p className={styles.scoreAndState}>
          {data.inning_state} {data.current_inning}
        </p>
      </div>
    );
  } else {
    return <p>{convertTime(data.game_datetime.substring(11))}</p>;
  }
}
